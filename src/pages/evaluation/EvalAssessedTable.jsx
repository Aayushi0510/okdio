...
const EvalAssessedTable = (props) => {
  ...
  const { TblContainer, TblHead, finalRecords } = useTable(
    tableData || [],
    TABLE_HEAD,
    "id",
    "evalAssessed",
  ...
  const fetchData = async (req) => {
    ...
    try {
      ...
      const response = await getMethod(
        API_URL.GET_EVALUATION_LIST + `/${req?.classId || ""}/2`,
  ...
  console.log("eval-ass-table", tableData);
    ...
    <CustomBackdrop loading={loading}>
      <Container maxWidth="xl">
        <Card sx={{ padding: "20px" }}>
          ...
          >
            <TblContainer name={"evaluationAss" || "table"}>
              <TblHead />
              <TableBody>
                {tableData &&
                  tableData?.length > 0 &&
...
export default EvalAssessedTable;
