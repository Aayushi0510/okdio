  ...
  return (
    <div className="bookActivity">
      <div className="bookActivity-head">
        ...
        <span className="bookActivity-head-title">{t("Drag Drop")}</span>
      </div>
                ...
                <div className="dragOptions">
                  {Array.isArray(question.data) && question?.data?.length > 0
                    ? question.data.map((option) => {
                        return (
                          <span key={option?.id} className="dragOptions-item">
                            {renderOption(option)}
                          </span>
                        );
                      })
